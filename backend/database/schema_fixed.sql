-- BeanScan Database Schema for Supabase (Fixed RLS Policies)
...
-- Create RLS (Row Level Security) policies - SIMPLIFIED VERSION
...
-- Allow public read access to basic tables
CREATE POLICY "Public read access" ON "Bean_Type" FOR SELECT USING (true);
CREATE POLICY "Public read access" ON "User" FOR SELECT USING (true);

-- Allow authenticated users to manage their own data
CREATE POLICY "Users manage own data" ON "Bean_Image" 
    FOR ALL USING (user_id IS NULL OR user_id = 1); -- Simplified for now

CREATE POLICY "Users manage own data" ON "Defect" 
    FOR ALL USING (true); -- Simplified for now

CREATE POLICY "Users manage own data" ON "Shelf_Life" 
    FOR ALL USING (true); -- Simplified for now

CREATE POLICY "Users manage own data" ON "History" 
    FOR ALL USING (true); -- Simplified for now
